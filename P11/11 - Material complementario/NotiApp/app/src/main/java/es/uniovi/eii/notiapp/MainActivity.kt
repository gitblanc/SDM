...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU){
            solicitarPermisosLauncher = registerForActivityResult(ActivityResultContracts.RequestPermission()) { permitido ->
                if (!permitido)
                    Toast.makeText(this, "No se mostrarán notificaciones", Toast.LENGTH_LONG).show()
                else
